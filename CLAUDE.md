...
1. **取得今天日期**：使用 `date '+%Y/%m/%d %A'` 確認今天日期
2. **查證星期幾**：寫日期+星期時，必須用程式確認
   ```bash
   # 確認特定日期是星期幾
   date -j -f '%Y-%m-%d' '2025-12-04' '+%A %Y-%m-%d'
   ```
3. **更新紀錄日期**：使用「實際上傳/更新日期」，而非「上課日期」
   - ✅ 今天 12/3 上傳第九週筆記 → changelog 日期寫 12/3
   - ❌ 第九週上課日是 11/27 → changelog 日期寫 11/27
4. **筆記檔名日期**：使用「上課日期」
   - ✅ `2025-11-27_基礎數學_課程筆記_整理版.md`（上課日期）
   - 這樣學生容易對應是哪一週的內容
